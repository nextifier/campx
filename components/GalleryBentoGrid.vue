<template>
  ...
  >
    <button
      v-for="(item, index) in item.photos.slice(0, 5)"
      @click="dialogs.updateDialog('experience-gallery', true)"
      :key="index"
      class="group bg-muted relative z-10"
      :class="{
        'col-span-4 row-span-2': item.photos.length == 1,
        'col-span-2 row-span-2': item.photos.length == 2,
        'col-span-2 first-of-type:row-span-2': item.photos.length == 3,
